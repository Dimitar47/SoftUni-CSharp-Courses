...
INSERT INTO Projects (Name, StartDate)
 ...
 INTO ProjectDesc
 FROM Projects 
Drop  table ProjectDesc

CREATE SEQUENCE seq_Projects_ProjectID
 AS INT
 START WITH 1
 INCREMENT BY 1


 SELECT NEXT VALUE FOR seq_Projects_ProjectID
...
-- Step 3: Display the updated salaries
SELECT Salary
FROM Employees
WHERE DepartmentID IN (
    SELECT DepartmentID 
...
UPDATE Employees
SET Salary = (SELECT b.Salary FROM Employees_Backup b WHERE Employees.EmployeeID = b.EmployeeID)
WHERE DepartmentID IN (
    SELECT DepartmentID 
    FROM Departments 
    WHERE [Name] IN ('Engineering', 'Tool Design', 'Marketing', 'Information Services')
